...
export function setupLeadsInputNamedRanges(): boolean {
  try {
    ...
    if (yearColIndex === 0 || monthColIndex === 0 || goalColIndex === 0 || revenueColIndex === 0) {
      Logger.log('Could not find all required columns in Leads Input sheet');
      Logger.log(`Year column: ${yearColIndex}, Month column: ${monthColIndex}, Goal column: ${goalColIndex}, Revenue column: ${revenueColIndex}`);
      return false;
    }
