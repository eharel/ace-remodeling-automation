...
function promptForNewProject() {
  ...
  const response = ui.prompt("Add New Project", "Enter the new project tab name (e.g., '999 Joe'):", ui.ButtonSet.OK_CANCEL);
  ...
  if (!match) {
    ui.alert("Please start with a project number followed by a client name. E.g., '123 John Doe'");
    return;
  }
...
function createProjectTabFromTemplate(tabName, projectNumber, clientName) {
  ...
  const newSheet = templateSheet.copyTo(ss).setName(tabName);
  ...
  try {
    ...
    if (advanceMaxRange) {
      ...
      Logger.log(`⚠️ Named range '${NR_ADVANCE_MAX}' not found in '${tabName}'`);
    }
  } catch (err) {
    Logger.log(`🚨 Error setting formula in '${NR_ADVANCE_MAX}': ${err.message}`);
  }
